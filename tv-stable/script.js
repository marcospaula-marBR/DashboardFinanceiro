...
function initEventListeners() {
    ...
    // Omie Sync Handlers
    const btnOmieSync = document.getElementById('btnOmieSync');
    if (btnOmieSync) {
        btnOmieSync.addEventListener('click', () => {
            const modal = new bootstrap.Modal(document.getElementById('omieModal'));

            // Pre-fill keys if available (from Service which now has hardcoded or stored)
            const omieService = new OmieService();
            const keyInput = document.getElementById('omieAppKey');
            const secretInput = document.getElementById('omieAppSecret');

            if (omieService.appKey) {
                keyInput.value = omieService.appKey;
                // Optional: Make read-only to indicate it's auto-configured
                // keyInput.readOnly = true; 
            }
            if (omieService.appSecret) {
                secretInput.value = omieService.appSecret;
            ...
            if (!document.getElementById('omieDtInicio').value) {
                document.getElementById('omieDtInicio').value = `${year}-01-01`;
            }
            if (!document.getElementById('omieDtFim').value) {
                document.getElementById('omieDtFim').value = `${year}-${month}-${day}`;
            }
    ...
    if (btnStartSync) {
        btnStartSync.addEventListener('click', async () => {
            const key = document.getElementById('omieAppKey').value.trim();
            const secret = document.getElementById('omieAppSecret').value.trim();
            const dtIni = document.getElementById('omieDtInicio').value;
            const dtFim = document.getElementById('omieDtFim').value;
            ...
            // Save credentials
            const omieService = new OmieService();
            omieService.setCredentials(key, secret);

            // Formatting dates for API? (YYYY-MM-DD -> DD/MM/YYYY handled by Service)

            // Close Modal
            const modalEl = document.getElementById('omieModal');
            ...
            statusEl.textContent = "⏳ Iniciando sincronização Omie...";

            try {
                // Call Sync
                // We pass a progress callback to update UI
                const result = await omieService.syncData(dtIni, dtFim, (msg) => {
                    console.log(msg);
                    statusEl.textContent = `⏳ ${msg}`;
                });

                // Handle New Result Structure { dreData, auditData }
                const data = result.dreData || result; // Fallback if simple array
                const auditData = result.auditData || [];

                if (data && data.length > 0) {
                    console.log("Dados Omie recebidos:", data.length);
                    processParsedData({ data: data });

                    statusEl.textContent = `✅ Omie: ${data.length} regist. (${auditData.length} mov).`;

                    // Update Last Update
                    const now = new Date();
                    document.getElementById('lastUpdate').textContent = `Atualizado via Omie: ${now.toLocaleDateString()} ${now.toLocaleTimeString()}`;

                    // Enable Audit Export
                    window.OMIE_AUDIT_DATA = auditData;

                    // Check/Create Audit Button dynamically
                    let btnAudit = document.getElementById('btnAuditExport');
                    if (!btnAudit) {
                        const container = statusEl.parentNode; // Usually the navbar or sidebar status area
                        btnAudit = document.createElement('button');
                        btnAudit.id = 'btnAuditExport';
                        btnAudit.className = 'btn btn-sm btn-warning ms-2';
                        btnAudit.innerHTML = '<i class="bi bi-table"></i> Auditoria';
                        btnAudit.title = "Baixar CSV detalhado dos lançamentos importados";
                        container.appendChild(btnAudit);
                    }

                    // Direct Inline Handler (Closure captures auditData)
                    btnAudit.onclick = function () {
                        try {
                            console.log("Exporting Audit Data...", auditData.length);
                            if (!auditData || auditData.length === 0) {
                                alert("Sem dados para exportar.");
                                return;
                            }

                            const headers = ["Origem", "Data", "Vencimento/Previsão", "Valor", "Categoria", "Projeto", "Cliente/Fornecedor", "Documento", "Observação"];
                            ...
                            auditData.forEach(row => {
                                const line = [
                                    ...
                                    row.Data,
                                    row.Vencimento,
                                    (row.Valor || 0).toString().replace('.', ','),
                                    `"${(row.CategoriaNome || '').toString().replace(/"/g, '""')}"`,
                                    `"${(row.ProjetoNome || '').toString().replace(/"/g, '""')}"`,
                                    row.Cliente || "",
                                    `"${(row.Documento || '').toString().replace(/"/g, '""')}"`,
                                    `"${(row.Observacao || '').toString().replace(/"/g, '""').replace(/\n/g, ' ')}"`
                                ];
                            ...
                            link.setAttribute("download", `Auditoria_Omie_${new Date().toISOString().slice(0, 10)}.csv`);
                    ...
                    statusEl.textContent = "⚠️ Nenhum dado encontrado.";
                    document.getElementById('loadingOverlay').classList.add('d-none');
                }
        ...
        state.charts.main = new Chart(ctxMain, {
            ...
            options: {
                ...
                plugins: {
                    ...
                    tooltip: mainTooltip,
                    datalabels: {
                        display: true, // Force global display if not overridden
                        clamp: true,
                        anchor: 'end',
                        align: 'top',
                        offset: 4,
            ...
            options: {
                ...
                scales: {
                    y: {
                        beginAtZero: true,
                        grid: { borderDash: [2, 4] },
                        grace: '5%' // Add extra space on top of axis
                    },
                    x: { grid: { display: false } }
                }
            }
...
function exportAuditCSV() {
    console.log("Iniciando exportação de auditoria...");
    // alert("Iniciando exportação..."); // Debug line if needed, but let's trust the logic first. 
    // Actually, if user says "nothing happens", avoiding alert might make them think it's still broken if it's fast.
    // Let's rely on the success interaction.

    const data = window.OMIE_AUDIT_DATA;
    if (!data || data.length === 0) {
        alert("Sem dados de auditoria disponíveis. (Tabela vazia)\nSincronize novamente.");
        return;
    }
    ...
    data.forEach(row => {
        const line = [
            ...
            row.Data,
            row.Vencimento,
            (row.Valor || 0).toString().replace('.', ','), // Format number to BR
            `"${(row.CategoriaNome || '').replace(/"/g, '""')}"`,
            `"${(row.ProjetoNome || '').replace(/"/g, '""')}"`,
            row.Cliente || "",
            `"${(row.Documento || '').replace(/"/g, '""')}"`,
            `"${(row.Observacao || '').replace(/"/g, '""').replace(/\n/g, ' ')}"`
        ];
        csvRows.push(line.join(","));
    });

    downloadCSV(csvRows.join("\n"), `Auditoria_Omie_${new Date().toISOString().slice(0, 10)}.csv`);
}

// ========================================
// TV DASHBOARD MODE CONTROLLER
// ========================================

let tvInterval = null;

function toggleTVMode() {
    const tvContainer = document.getElementById('tv-dashboard');
    const isHidden = tvContainer.classList.contains('d-none');
    ...
    if (isHidden) {
        ...
        // TURN OFF
        tvContainer.classList.add('d-none');
        document.body.style.overflow = 'auto';
...
function startTVClock() {
    const updateTime = () => {
        const now = new Date();
        document.getElementById('tvClock').textContent = now.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' });
    };
    updateTime();
    tvInterval = setInterval(updateTime, 1000);
}

function updateTVContent() {
    const container = document.getElementById('tvScrollParams');
    container.innerHTML = '';
    ...
    const revenue = (m.total_entradas || 0) + (m.outras_entradas || 0);
    const getPerc = (val) => {
        if (!revenue || revenue === 0) return 0;
        return (val / revenue) * 100;
    };

    // 1. GLOBAL KPIS (As Percentages mostly)
    // User requested "percentuais já existentes nos indicadores". 
    // Usually means Vertical Analysis for Costs/Expenses, and Margin % for Result.

    const kpis = [
        // For Revenue, % is 100%, maybe show Value? Or Growth? Let's show Result Margin.
        { label: 'Margem Líquida', val: (m.perc_lucro || 0).toFixed(1) + '%', type: 'kpi', color: 'tv-val-positive' },
        { label: 'Margem Bruta', val: getPerc((m.total_entradas || 0) - (m.total_custos || 0)).toFixed(1) + '%', type: 'kpi', color: 'tv-val-positive' },

        // Costs & Expenses as % of Revenue
        { label: 'Custos Oper.', val: getPerc(m.total_custos).toFixed(1) + '%', type: 'kpi', color: 'tv-val-negative' },
        { label: 'Despesas', val: getPerc(m.total_despesas).toFixed(1) + '%', type: 'kpi', color: 'tv-val-negative' },

        // Personnel is usually important
        { label: 'Pessoal', val: getPerc(m.pessoal).toFixed(1) + '%', type: 'kpi', color: 'tv-val-neutral' },

        // FCL is hard to show as %, keep Abs? Or User implied ALL as %?
        // "mostrando valores absolutos e queria isso em percentuais"
        // Let's mix: Margins are %, FCL maybe keep Value or % of Rev? Let's try % of Rev for consistency if possible, or Abs if it makes more sense.
        // FCL % of Revenue IS a thing (Free Cash Flow Margin).
        { label: 'Margem FCL', val: getPerc(m.fcl).toFixed(1) + '%', type: 'kpi', color: m.fcl >= 0 ? 'tv-val-positive' : 'tv-val-negative' }
    ];
    ...
    kpis.forEach(k => {
        cardsHTML += `
            <div class="tv-card ${k.type}">
                <div class="tv-card-header"><i class="bi bi-pie-chart"></i> ${k.label}</div>
                <div class="tv-card-value ${k.color}">${k.val}</div>
                <div class="tv-card-sub">Sobre Faturamento</div>
            </div>
        `;
    });

    // 2. EQUIPMENT ANALYSIS (Per Unit Only)
    // "acrescendo a análise por máquinas que já existem dentro do DRE (index)... mas só o valor por máquina"
    // We use the same categories as openPorMaquinaModal
    const qtdEquip = m.total_equipamentos || 1; // Avoid div by zero

    const equipItems = [
        // { title: 'Receita Média', value: m.total_entradas, icon: 'bi-graph-up-arrow', color: 'tv-val-positive' }, // Maybe confusing with Total?
        { title: 'Resultado Médio', value: m.resultado, icon: 'bi-bullseye', color: m.resultado >= 0 ? 'tv-val-positive' : 'tv-val-negative' },
        { title: 'Custo Médio', value: m.total_custos, icon: 'bi-gear', color: 'tv-val-negative' },
        { title: 'Despesa Média', value: m.total_despesas, icon: 'bi-calculator', color: 'tv-val-negative' },
        { title: 'Manut. Corretiva', value: m.corretiva, icon: 'bi-tools', color: 'tv-val-negative' },
        { title: 'Manut. Preventiva', value: m.preventiva, icon: 'bi-shield-check', color: 'tv-val-negative' },
        { title: 'Combustível', value: m.combustivel || 0, icon: 'bi-fuel-pump', color: 'tv-val-negative' }, // If available
        { title: 'Peças', value: m.pecas || 0, icon: 'bi-gear-wide', color: 'tv-val-negative' }     // If available
    ];

    equipItems.forEach(eq => {
        if (eq.value === 0) return; // Skip empty
        const perUnit = eq.value / qtdEquip;

        cardsHTML += `
            <div class="tv-card equip">
                <div class="tv-card-header"><i class="bi ${eq.icon}"></i> ${eq.title}</div>
                <div class="tv-card-value ${eq.color}">${formatCurrency(perUnit)}</div>
                <div class="tv-card-sub">Por Equipamento</div>
            </div>
        `;
    });

    // 3. Info Update
    document.getElementById('tvUpdateInfo').textContent = `Atualizado: ${new Date().toLocaleDateString('pt-BR')}`;

    // INJECT CONTENT (DUPLICATED FOR MARQUEE LOOP)
    // We insert the content TWICE to create the seamless loop effect
    container.innerHTML = cardsHTML + cardsHTML;
}

// Remove old equipment calculation, not needed anymore
/* function calculateEquipmentResults() ... removed */
