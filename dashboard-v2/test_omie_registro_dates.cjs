  ...
  if (res.faultstring) {
    ...
    if (records.length > 0) {
      records.slice(0, 5).forEach((r, idx) => {
        console.log(` [${idx}] ID: ${r.codigo_lancamento_omie} | Venc: ${r.data_vencimento} | Reg/Entrada: ${r.data_entrada} | Inc: ${r.info.dInc} | Alt: ${r.info.dAlt}`);
      });
    }
  }
}
...
async function run() {
  ...
  await test('filtrar_por_registro_de / ate', {
    pagina: 1, registros_por_pagina: 5,
    filtrar_por_registro_de: de, filtrar_por_registro_ate: ate
  });
}
