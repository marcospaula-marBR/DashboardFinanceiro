...
async function check() {
  const { data, error } = await supabase.from('employees').select('id, full_name, status, status_end_date').ilike('full_name', '%Carlos%');
  console.log("Carlos (PROD):", data);

  const { data: d2 } = await supabase.from('employees').select('id, full_name, status, status_end_date').ilike('full_name', '%F%bio%');
  console.log("Fabio (PROD):", d2);
}
