...
import { RecebiveisAuditEntry } from '@/types/recebiveis';
import { ShieldAlert, Check, X, CheckCheck, AlertTriangle, Calendar, User, FileText, ChevronDown, ChevronUp, DollarSign } from 'lucide-react';

interface Props {
  isOpen: boolean;
  onClose: () => void;
  entries: RecebiveisAuditEntry[];
  onApproveEntry: (id: string) => void;
  onRejectEntry: (id: string) => void;
  onApproveAll: () => void;
}

export const RecebiveisAuditModal: React.FC<Props> = ({
  isOpen,
  onClose,
  entries,
  onApproveEntry,
      ...
      <div className={styles.modalContent} onClick={e => e.stopPropagation()} style={{ maxWidth: '900px' }}>
        ...
        <div className={styles.modalHeader}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
            ...
            <div>
              <h3 className={styles.modalTitle}>Auditoria de Sincronização & Aceite de Mudanças Omie</h3>
              <p style={{ margin: 0, fontSize: '0.75rem', color: '#94a3b8' }}>
                Revisão completa dos lançamentos do Omie com Razão Social, Vencimento, Retenções e Líquido antes de gravar no banco
              </p>
            </div>
          </div>
        ...
        <div style={{ background: '#0b1120', padding: '0.85rem 1rem', borderRadius: '10px', marginBottom: '1.25rem', border: '1px solid rgba(245, 158, 11, 0.3)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            ...
            <div style={{ fontSize: '0.72rem', color: '#94a3b8', marginTop: '2px' }}>
              O banco de dados só é atualizado após a sua revisão e aprovação explícita de cada item.
            </div>
          </div>
        ...
        <div style={{ maxHeight: '480px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '1rem', paddingRight: '4px', marginBottom: '1.25rem' }}>
          {pendingEntries.length === 0 ? (
            <div style={{ padding: '3rem 2rem', textAlign: 'center', color: '#64748b', fontSize: '0.85rem', background: 'rgba(30, 41, 59, 0.2)', borderRadius: '10px' }}>
              ...
              <div style={{ color: '#94a3b8', fontSize: '0.75rem', marginTop: '4px' }}>Os recebimentos no banco de dados estão perfeitamente atualizados com o Omie ERP.</div>
            </div>
          ) : (
            pendingEntries.map(item => {
              const d = item.new_data;
              const isExpanded = expandedEntries[item.id] ?? true;
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', marginBottom: '0.85rem', fontSize: '0.78rem' }}>
                      ...
                      <div style={{ background: 'rgba(30, 41, 59, 0.4)', padding: '0.75rem', borderRadius: '8px', border: '1px solid rgba(255, 255, 255, 0.05)' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem', color: '#cbd5e1' }}>
                          ...
                          <div><strong>Data de Emissão:</strong> {formatDate(d.date_issue)}</div>
                          <div><strong>Data de Vencimento:</strong> {formatDate(d.date_due)}</div>
                          <div>
                            <strong>Status no Omie:</strong>{' '}
                            <span style={{ color: d.status === 'Pago' ? '#10b981' : '#f59e0b', fontWeight: 700 }}>
                              {d.status === 'Pago' ? `PAGO (${formatDate(d.date_payment)})` : 'PENDENTE'}
                            </span>
                          </div>
                        </div>
                      </div>

                      {/* Bloco 2: Impostos & Classificação B2G/B2B/B2C */}
                      <div style={{ background: 'rgba(30, 41, 59, 0.4)', padding: '0.75rem', borderRadius: '8px', border: '1px solid rgba(255, 255, 255, 0.05)' }}>
                        <div style={{ fontWeight: 700, color: '#10b981', marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.3rem', fontSize: '0.75rem' }}>
                          <DollarSign size={13} />
                          <span>Tributação Retida & Classificação</span>
                        </div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem', color: '#cbd5e1' }}>
                          ...
                          <div style={{ fontSize: '0.72rem', color: '#8b5cf6', marginTop: '2px' }}>
                            <strong>Comissões:</strong> {d.commission && d.commission.has_commission ? `SIM (${formatCurrency(d.commission.total_commission_value)})` : 'Desativadas'}
                          </div>
                        </div>
                      </div>
