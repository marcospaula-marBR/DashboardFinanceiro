...
export interface OmieCandidate {
  ...
  impostos: number;
  status: string;
  status_omie: string; // RECEBIDO | ABERTO | CANCELADO | VENCIDO
  // Impostos
  tax_ir: number;
  tax_pis: number;
  tax_cofins: number;
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[92vh] flex flex-col border border-slate-100 animate-in zoom-in-95 duration-200">
              ...
              {candidates.length === 0 ? (
                <div className="py-12 text-center">
                  ...
                  <p className="text-xs text-slate-400 mt-1">
                    Tente ajustar as datas ou o critério de busca.
                  </p>
                </div>
              ) : (
                candidates.map((c) => {
                        ...
                        <span
                          className={`text-[10px] font-black px-2 py-0.5 rounded-full shrink-0 ${
                            c.status === "Pago"
                              ...
                              : c.status === "Cancelado"
                              ? "bg-slate-100 text-slate-500 border border-slate-300/60"
                          ...
                          {c.status === "Pago" && c.date_payment ? (
                            <>
                              ...
                              <p className="text-xs font-bold text-emerald-700">
                                {formatDateDisplay(c.date_payment)}
                              </p>
                            </>
                          ) : (
                            <>
                        ...
                        <div className="px-4 pb-4 border-t border-slate-100 pt-3 space-y-3">
                          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                            <div className="bg-slate-50 rounded-lg px-3 py-2 col-span-2 sm:col-span-1">
                              <p className="text-[9px] font-black text-slate-400 uppercase">
                                Cliente (Razão Social / ID Omie)
                              </p>
                              <p className="text-xs font-bold text-slate-800 mt-0.5">
                                {c.client_name}
                              </p>
                              <p className="text-[10px] text-slate-400">
                                Cód. Omie: {c.client_code}
                              </p>
                            </div>
                            ...
                            <div className="bg-slate-50 rounded-lg px-3 py-2">
                              <p className="text-[9px] font-black text-slate-400 uppercase">
                                Contrato / Distribuição Omie
                              </p>
                              <p className="text-xs font-bold text-slate-800 mt-0.5 truncate">
                                {c.contract_name || "—"}
                              </p>
                              <p className="text-[10px] text-slate-400">
                                {c.contract_number || "Sem nº de contrato"}
                              </p>
                            </div>
                          </div>
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                            {[
                              ...
                              {
                                label:
                                  c.status === "Pago" ? "💳 Recebimento" : "🕒 Previsão",
                                value: c.date_payment,
                              },
                            ].map(({ label, value }) => (
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                            {[
                              ...
                              <div key={label} className="bg-slate-50 rounded-lg px-3 py-2">
                                ...
                                <p className={`text-xs font-black mt-0.5 ${color}`}>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                              ...
                              {(c.projeto_nome || c.projeto_code) && (
                                <div className="bg-violet-50/60 rounded-lg px-3 py-2">
                                  <p className="text-[9px] font-black text-violet-400 uppercase">
                                    Projeto Omie
                                  </p>
                                  <p className="text-xs font-bold text-slate-800 mt-0.5">
                                    {c.projeto_nome || "—"}
                          ...
                          <div className="bg-red-50/60 rounded-lg px-3 py-2">
                            <p className="text-[9px] font-black text-red-500 uppercase mb-1">
                              Detalhamento Fiscal Retido
                            </p>
                            <div className="flex flex-wrap gap-x-5 gap-y-1 text-[11px] text-slate-600 font-semibold">
                              <span>
                                IR: <strong>{formatCurrency(c.tax_ir)}</strong>
                              </span>
                              <span>
                                PIS: <strong>{formatCurrency(c.tax_pis)}</strong>
                              </span>
                              <span>
                                COFINS: <strong>{formatCurrency(c.tax_cofins)}</strong>
                              </span>
                              <span>
                                ISS: <strong>{formatCurrency(c.tax_iss)}</strong>
                              </span>
                              <span>
                                INSS: <strong>{formatCurrency(c.tax_inss)}</strong>
                              </span>
                              {c.tax_csll > 0 && (
                                <span>
                                  CSLL: <strong>{formatCurrency(c.tax_csll)}</strong>
                                </span>
                              )}
                            </div>
                          </div>
                          ...
                          <div>
                            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-wider mb-1.5">
                              ...
                              <span className="normal-case font-semibold text-amber-600 ml-1">
                                (sem vínculo o lançamento será ignorado no rateio de comissões)
                              </span>
                            </label>
                            <select
                            ...
                            >
                              <option value="">
                                — Não vincular a contrato (importar sem rateio) —
                              </option>
                              {contratos.map((ct) => (
        ...
        {step === "importing" && (
          <div className="flex-1 flex flex-col items-center justify-center py-16 gap-4">
            ...
            <p className="text-sm font-bold text-slate-600">
              Gravando lançamentos no banco de dados…
            </p>
