...
def main():
    status, payments = http("GET", "/rest/v1/loan_payments?employee_id=eq.09f4635a-5ed6-4d0a-b9b1-f2637e34be1a&select=*")
    for p in sorted(payments, key=lambda x: x.get('due_date', '')):
        if '2026-06' in p['due_date']:
            print(f"Due: {p['due_date']} | Amt: {p['amount']} | Status: {p['status']} | Paid Date: {p.get('paid_date')} | ID: {p['id']}")
